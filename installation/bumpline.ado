...
syntax varlist(min=2 max=2) [if] [in], by(varname)  ///
	...
	[ XLABSize(string) YLABSize(string) XLABAngle(string) ] ///
...
preserve	
	...
	*levelsof `xvar'
	...
	if "`select'"=="any" | "`select'"=="" {
		drop if _rank > `top'  // hard ranks (only top 10)
	}
	...
	*** for the sigmoid box
	sort `by' `xvar' _seq
	bysort _id: gen double _xnorm =  ((_n - 1) / (`newobs' - 1))
	...
	foreach x of local lvl {
		...
		local lines `lines' (line    _yval _xval if _group==`x', lw(`lwidth') lc("`r(p`clr')'") )
		
		local marks `marks' (scatter _rankrev `xvar' if _group==`x' & tagxy==1, msym(`msymbol') mlwidth(`mlwidth') msize(`msize')  mc("`r(p`clr')'") mlc("`r(p`clr')'") )
	...
	twoway ///
		...
		xtitle("") ytitle("") ///
		ylabel(`ymin'(1)`ymax', valuelabels labsize(`ylabsize') ) ///
